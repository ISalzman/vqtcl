...
int TestBit (Seq_p bitmap, int row) {
  if (bitmap != NULL && row < bitmap->count) {
    const uint8_t *bits = bitmap->data[0].p;
...
Seq_p NewSequenceNoRef (int count, SeqType_p type, int bytes) {
  ...
  /* default for data[1].i is to contain the number of extra bytes allocated */
  seq->data[0].p = seq + 1;
  seq->data[1].i = bytes;
...
Seq_p ResizeSeq (Seq_p seq, int pos, int diff, int elemsize) {
  ...
  if (nlimit > 0) {
    result = IncRefCount(NewSequence(seq->count, seq->type, nlimit * elemsize));
...
static struct SeqType ST_String = { "string", StringGetter, 0100 };
...
Seq_p NewStrVec (int istext) {
  ...
  /* data[0] starts as two buffers, then becomes a vector of string pointers */
  /* data[1] is not used */
...
Seq_p FinishStrVec (Seq_p seq) {
  ...
  ptrs[r] = fill; /* past-end mark is needed to provide length of last entry */
...
Column OmitColumn (Column omit, int size) {
  int i, *data, j = 0, outsize = size - omit.seq->count;
  Seq_p seq, map;
  
  /* FIXME: bit wasteful for large views, consider sorting input col instead */
  map = NewBitVec(size);
...
Column CoerceColumn (ItemTypes type, Object_p obj) {
  ...
  if (n == 0 || GetItem(0, &item) == type)
    return column;
    
  switch (type) {
    case IT_int:    seq = NewIntVec(n, (void*) &data); break;
    case IT_wide:   seq = NewSequence(n, PickIntGetter(64), 8 * n); break;
    case IT_float:  seq = NewSequence(n, FixedGetter(4,1,1,0), 4 * n); break;
    case IT_double: seq = NewSequence(n, FixedGetter(8,1,1,0), 8 * n); break;
  ...
  for (i = 0; i < n; ++i) {
    ...
    switch (type) {
      ...
      case IT_bytes:  AppendToStrVec(item.u.ptr, item.u.len, out.seq); break;
...
int CastObjToItem (char type, Item_p item) {
  switch (type) {
    ...
    case 'n': {
      ...
      for (r = 0; r < rows; ++r) {
        /* FIXME: this only works if the input is a list, not a column! */
        data[r] = ColumnByName(meta, GetColItem(r, column, IT_object).o);
        if (data[r] < 0)
          return 0;
      }
...
typedef struct EmitInfo {
  int64_t        position;  /* emit offset, track > 2 Gb even on 32b platform */
  ...
  switch (type) {
    ...
    case IT_int:
      ...
      if (eip->diff && rows >= 128 && rows == bufsize * 8) {
        ...
        /* only keep the compressed form if it is under 80% of plain bitmap */
...
static Int_t EmitComplete (EmitInfo_p eip, View_p view) {
  ...
  SetBigEndian32(tail+8, ((eip->diff ? 0x90 : 0x80) << 24) + tailpos - rootpos);
...
static Seq_p MappedFixedCol (MappedFile_p map, int rows, const char **nextp, int isreal) {
  ...
  /* if the bit count is too low-for single-bit vectors then it's compressed */
...
static ItemTypes MappedBytesGetter (int row, Item_p item) {
  ...
  if (offsets[row] >= 0)
    item->u.ptr = (const uint8_t*) data + offsets[row];
  else {
    const char *next = data - offsets[row];
    item->u.len = GetVarInt(&next);
    item->u.ptr = (const uint8_t*) data + GetVarInt(&next);
  }
...
View_p MappedToView (MappedFile_p map, View_p base) {
  ...
  if (rootoff < 0) {
    ...
    /* FIXME: rollover at 2 Gb, probably needs: if (rootoff > datalen) ... */
  }
...
static void FillHashInfo (HashInfo_p info, View_p view) {
  ...
  rows = ViewSize(view);
  mapcol = SeqAsCol(NewIntVec(rows, NULL)); /* worst-case, don't know #groups */
...
static void FillJoinInfo (HashInfo_p info, View_p left, View_p right) {
  ...
  nleft = ViewSize(left);
  mapcol = SeqAsCol(NewIntVec(nleft, NULL)); /* worst-case don't know #groups */
