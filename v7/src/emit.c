...
static int EmitFixCol (EmitInfo *eip, vqSlot column, vqType type) {
    int r, rows = vSize(column.v), *tempvec;
    void *buffer;
    intptr_t bufsize;
...
static void EmitVarCol (EmitInfo *eip, vqSlot column, int istext, int rows) {
    int r, bytes;
    intptr_t buflen;
    Buffer buffer;
    vqColumn sizes = new_datavec(VQ_int, rows);
...
static void EmitSubCol (EmitInfo *eip, vqSlot column, int describe, int rows) {
    int r;
    Buffer newcolbuf;
    Buffer *origcolbuf;
        ...
        for (c = 0; c < vSize(meta); ++c) {                          
            ...
            switch (type) {
                ...
                case VQ_view:
                    ...
                    EmitSubCol(eip, view->col[c], vSize(subv) == 0, rows);
                    break;
                default: assert(0);
            }
        }
...
static int e_describe (vqEnv env) {
    Buffer buffer;
    vqView m = check_view(env, 1);
    if (vwMeta(m) != vwMeta(vwMeta(m)))
        m = vwMeta(m);
    InitBuffer(&buffer);
    MetaAsDesc(m, &buffer);
    lua_pushlstring(env, BufferAsPtr(&buffer, 1), BufferFill(&buffer));
    ReleaseBuffer(&buffer, 0);
    return 1;
}
...
static int e_emit (vqEnv env) {
    intptr_t n;
    luaL_Buffer b;
    vqView v = check_view(env, 1);
    luaL_buffinit(env, &b);   
    n = ViewSave(v, &b, 0, EmitDataFun);
    luaL_pushresult(&b);
    if (n < 0)
        return luaL_error(env, "error in view emit");
    return 1;
}

static const struct luaL_reg vqlib_emit[] = {
    {"describe", e_describe},
    {"emit", e_emit},
    {0, 0},
};
