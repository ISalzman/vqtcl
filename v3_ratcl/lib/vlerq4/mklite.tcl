...
# partial emulation of the mk::* commands
  ...
  proc file {cmd db args} {
    ...
    switch $cmd {
      open { set mkdb(?) ""; set mkdb($db) [vlerq open $file]; return $db }
      close { set mkdb(?) ""; unset mkdb($db); return }
      views { return [vlerq get $mkdb($db) @ * 0] }
    }
    error "mkfile $cmd?"
  }
  ...
  proc select {path args} {
    ...
    for {set i 0} {$i < $n} {incr i} {
      ...
      set c [vlerq get $vw * [lindex $args $i]]
