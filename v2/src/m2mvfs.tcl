...
# data file when unmounted, using ratcl calls.  Does not support deletions,
# renames, and such.  Can only be used with a single mountpoint at a time.
#
# 1.0	initial release
# 1.1	fix nested vdef in makeDirsView 
# 1.2	add "-to" arg to save operation
# 1.3	require ratcl 1.0

package provide vfs::m2m 1.3
package require vfs
package require ratcl 1.0
...
namespace eval vfs::m2m {
  ...
  namespace eval v {
    ...
    # the following lists are all indexed by the same directory ordinal number
...
# public
  ...
  proc Unmount {db local} {
    ...
    if {$v::debug} {
      puts "local [file tail $local] #dirs [llength $v::dname]\
			    #files [llength [eval concat $v::files]]"
    }
    if {$v::outfn ne ""} {
      set fd [::open $v::outfn a]
      view [makeDirsView] save -to $fd
      ::close $fd
    }
...
# private
  proc makeDirsView {} {
    set d {}
    foreach n $v::dname p $v::prows f $v::files {
      lappend d $n $p [vdef name size:I date:I contents:B [eval concat $f]]
    }
    vdef dirs:V [list [vdef name parent:I files:V $d]] ;# avoid shimmering!
  }
...
# methods
  ...
  proc fileattributes {db root path args} {
    switch -- [llength $args] {
      ...
      1 { return [eval [linsert $args 0 ::vfs::attributesGet $root $path]] }
      2 { return [eval [linsert $args 0 ::vfs::attributesSet $root $path]] }
    }
  }
  proc open {db file mode permissions} {
	...
	return [list $fd [list ::vfs::m2m::doClose $db $dpos $fpos $fd]]
      ...
      default { error "illegal access mode: $mode" }
    }
  }
